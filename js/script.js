...
// --- ВСТАВЛЯЕМ ВАШУ ЛОГИКУ СЮДА ---
    let max_speed = 222; 
    let start_offset = 625; 
    let end_offset = 250;   
    let range = start_offset - end_offset; 

    // Считаем положение стрелки
    let meter_count = start_offset - (speed * (range / max_speed));

    // Ограничиваем, чтобы стрелка не вылетала за пределы
    if (meter_count < end_offset) meter_count = end_offset;
    if (meter_count > start_offset) meter_count = start_offset;

    // Применяем результат
    document.getElementById("speed-meter").style.strokeDashoffset = meter_count;
    // ----------------------------------

    // Дальше ваш остальной код (обновление одометра, бензина и т.д.)
    document.getElementById('speed-odometer').innerHTML = odometer;
    document.getElementById('speed-fuel').innerHTML = fuel;
});
